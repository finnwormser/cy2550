 ...
 */
void merge_sort_aux(long nums[], int from, int to, long target[], int thread_id) {
  if (to - from <= 1) {
    return;
  }
  ...
  // initializing arg struct 
  thread_args args = {target, from, mid, nums, (thread_id + 1) / 2};

 
  // create thread using arg struct 
  assert(0 == pthread_create(&thread, NULL, merge_sort_wrapper, &args));
 
  // sorting the rest of the arguments
  merge_sort_aux(target, mid, to, nums, thread_id / 2);

  void* ret; 
  // wait for thread to finish
  assert(0 == pthread_join(thread, &ret));
}
  // merge all arrays
...
// merge sort wrapper function to be used by thread
// calls on merge_sort_aux using args from struct
void* merge_sort_wrapper(void* args) {
  // arg pointer
  thread_args* t_args = (thread_args *)args;
  merge_sort_aux(t_args->nums, t_args->from, t_args->to, t_args->target, t_args->thread_id);
  // wait for NULL
  pthread_exit(NULL);
}
